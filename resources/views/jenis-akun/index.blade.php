...
@section('header')
    <div class="page-header">
        <div class="row align-items-center">
            <div class="col">
                <h1 class="page-header-title">Jenis Akun</h1>
            </div>
        </div>
    </div>
@endsection
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <button class="btn btn-sm btn-danger btn-active-light-danger ms-1 delete"
                                            data-kode="{{ $jenisAkun->getKey() }}">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="add-modalLabel">Tambah Jenis Akun</h5>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" data-jenis_akun=""
                        class="btn btn-primary d-flex gap-2 align-items-center simpan store-jenis-akun">
    ...
    <script>
        $(document).on('ready', function() {
            ...
            $('.add').click(function(e) {
                ...
                $('#add-modal .simpan').data('jenis-akun', '');
                $('#add-modal .simpan').removeClass('update-jenis-akun');
                $('#add-modal .simpan').addClass('store-jenis-akun');
                $('#add-modal #add-modalLabel').text('Tambah Jenis Akun');
            });

            $('#add-modal').on('click', '.store-jenis-akun', function(e) {
                e.preventDefault();
                var postData = new FormData($(".form-add-modal")[0]);
                $('#add-modal .simpan .loading').show();
                $.ajax({
                    type: 'POST',
                    url: "/jenis-akuns/store",
                    processData: false,
                    contentType: false,
                    data: postData,
            ...
            $('#datatable').on('click', '.edit', function(e) {
                e.preventDefault();
                var target = this;
                var jenis_akun = $(target).data('kode');
                $($(target).find('.loading')).show();
                $.ajax({
                    type: "POST",
                    url: `/jenis-akuns/${jenis_akun}/edit`,
                    dataType: "JSON",
                    success: function(response) {
                        $($(target).find('.loading')).hide();
                        $('.form-add-modal input[name="nama"]').val(response.nama);
                        $('#add-modal .simpan').data('jenis-akun', response.key);
                        $('#add-modal .simpan').addClass('update-jenis-akun');
                        $('#add-modal .simpan').removeClass('store-jenis-akun');
                        $('#add-modal #add-modalLabel').text('Ubah Jenis Akun');
            ...
            $('#add-modal').on('click', '.update-jenis-akun', function(e) {
                e.preventDefault();
                var postData = new FormData($(".form-add-modal")[0]);
                $('#add-modal .simpan .loading').show();
                var jenis_akun = $(this).data('jenis-akun');
                $.ajax({
                    type: 'POST',
                    url: `/jenis-akuns/${jenis_akun}/update`,
                    processData: false,
                    contentType: false,
                    data: postData,
                    success: function(response) {
                ...
                Swal.fire({
                    ...
                    preConfirm: () => {
                        return new Promise(function(resolve) {
                            $.ajax({
                                    type: "DELETE",
                                    url: `/jenis-akuns/${jenis_akun}/destroy`,
                                    dataType: 'JSON',
                                })
